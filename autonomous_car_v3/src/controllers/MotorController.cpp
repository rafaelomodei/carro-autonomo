...
namespace {
constexpr int kInactive = LOW;
constexpr int kActive = HIGH;
}
...
void MotorController::setMotorState(int forward_pin, int backward_pin, bool forward_active, bool backward_active) {
    digitalWrite(forward_pin, forward_active ? kActive : kInactive);
    digitalWrite(backward_pin, backward_active ? kActive : kInactive);
}
