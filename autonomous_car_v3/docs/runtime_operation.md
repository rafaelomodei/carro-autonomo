...
## Validacao usada nesta documentacao

Esta documentacao foi revisada contra o codigo atual e validada com um build limpo local:

```bash
cmake -S autonomous_car_v3 -B autonomous_car_v3/build_codex
cmake --build autonomous_car_v3/build_codex
./autonomous_car_v3_tests
```

Resultado observado durante a validacao:

- `autonomous_car_v3_tests` compilou com sucesso em build limpo
- 37 testes passaram
- o alvo de hardware nao foi gerado nesse ambiente porque `wiringPi` nao estava disponivel, o que esta alinhado com a implementacao atual

Para tuning do controlador e leitura do painel local, consulte tambem [`pid_control.md`](./pid_control.md).
