...
const cv::Scalar kUnknownColor{170, 170, 170};
const cv::Scalar kLabelBackground{16, 16, 16};

constexpr int kCanvasWidth = 1380;
constexpr int kCanvasHeight = 860;
constexpr int kPadding = 18;
constexpr int kGap = 18;
constexpr int kHeaderHeight = 170;
constexpr int kFooterHeight = 140;
...
cv::Scalar detectionColor(TrafficSignalId signal_id) {
    return signal_id == TrafficSignalId::Unknown ? kUnknownColor : kRawColor;
}
...
void drawDetectionBox(cv::Mat &image, const SignalDetection &detection, const cv::Size &source_size,
                      const cv::Rect &content_rect, const cv::Scalar &color, int thickness,
                      const std::string &prefix = {}) {
    TrafficSignBoundingBox scaled_box =
        scaleBoundingBox(detection.bbox_frame, source_size, content_rect.size());
    scaled_box = clampBoundingBox(translateBoundingBox(scaled_box, content_rect.tl()),
    ...
    const std::string tag =
        prefix.empty()
            ? detection.display_label + " " + formatDouble(detection.confidence, 2)
            : prefix + ": " + detection.display_label + " " +
                  formatDouble(detection.confidence, 2);
    drawLabelTag(image, {rect.x, std::max(18, rect.y - 4)}, tag, color);
}

void drawDetectionBoxOnRoiTile(cv::Mat &image, const SignalDetection &detection,
...
std::string activeSummary(const TrafficSignFrameResult &result) {
    if (result.active_detection.has_value()) {
        return result.active_detection->display_label + " " +
               formatDouble(result.active_detection->confidence, 2);
    }

    if (result.candidate.has_value()) {
        return "Cand. " + result.candidate->display_label + " " +
               formatDouble(result.candidate->confidence, 2);
    }
...
cv::Mat TrafficSignDebugRenderer::render(const TrafficSignFrameResult &result,
    ...
    const int content_y = header_area.y + header_area.height + kGap;
    const int content_height = canvas.rows - content_y - kFooterHeight - kGap - kPadding;
    const int left_width = static_cast<int>(std::lround((canvas.cols - kPadding * 2 - kGap) * 0.58));
    const int right_width = canvas.cols - kPadding * 2 - kGap - left_width;
    const int right_tile_height = (content_height - kGap) / 2;
    const cv::Rect frame_tile_area(kPadding, content_y, left_width, content_height);
    const cv::Rect roi_tile_area(frame_tile_area.x + frame_tile_area.width + kGap, content_y,
                                 right_width, right_tile_height);
    const cv::Rect model_tile_area(roi_tile_area.x, roi_tile_area.y + roi_tile_area.height + kGap,
                                   right_width, right_tile_height);
    const cv::Rect footer_area(kPadding, model_tile_area.y + model_tile_area.height + kGap,
    ...
    cv::putText(canvas, "Traffic Sign Service", {header_area.x + 16, header_area.y + 28},
                cv::FONT_HERSHEY_SIMPLEX, 0.74, kTextPrimary, 2, cv::LINE_AA);
    cv::putText(canvas,
                "Frame completo, ROI original e input preprocessado enviado ao backend",
    ...
    const std::vector<std::string> header_lines = {
        ...
        "Fonte: " + runtime_metrics.source_label + " | modo: " + runtime_metrics.source_mode +
            " | backend: " + runtime_metrics.backend,
        "Deteccoes brutas: " + std::to_string(result.raw_detections.size()) +
            " | FPS: " + formatDouble(runtime_metrics.service_fps, 1) +
            " | Inferencia: " + formatDouble(runtime_metrics.inference_ms, 1) +
            " ms | idade: " + formatAgeLabel(runtime_metrics.sign_result_age_ms),
        "Frames recebidos: " + std::to_string(runtime_metrics.frames_received) +
            " | processados: " + std::to_string(runtime_metrics.frames_processed) +
            " | descartados: " + std::to_string(runtime_metrics.dropped_frames),
    ...
    drawTileFrame(canvas, frame_tile_area, "Frame completo");
    ...
    const cv::Rect frame_content(frame_tile_area.x + 12, frame_tile_area.y + 36,
                                 frame_tile_area.width - 24, frame_tile_area.height - 48);
    ...
    cv::Rect frame_image_rect;
    cv::Mat frame_canvas = fitImageToCanvas(result.debug_frame, frame_content.size(), &frame_image_rect);
    frame_canvas.copyTo(canvas(frame_content));
    if (!result.debug_frame.empty()) {
        const cv::Rect frame_bounds(frame_content.x + frame_image_rect.x,
                                    frame_content.y + frame_image_rect.y,
                                    frame_image_rect.width, frame_image_rect.height);
        const cv::Rect roi_rect = result.roi.frame_rect;
        const TrafficSignBoundingBox roi_bbox{roi_rect.x, roi_rect.y, roi_rect.width, roi_rect.height};
        TrafficSignBoundingBox scaled_roi =
            scaleBoundingBox(roi_bbox, result.debug_frame.size(), frame_bounds.size());
        scaled_roi = clampBoundingBox(translateBoundingBox(scaled_roi, frame_bounds.tl()),
                                      cv::Rect(0, 0, canvas.cols, canvas.rows));
        cv::rectangle(canvas, toCvRect(scaled_roi), kRoiColor, 2, cv::LINE_AA);
        drawLabelTag(canvas, {toCvRect(scaled_roi).x, std::max(18, toCvRect(scaled_roi).y - 4)},
                     "ROI", kRoiColor);

        const cv::Size source_size = result.debug_frame.size();
        for (const auto &detection : result.raw_detections) {
            drawDetectionBox(canvas, detection, source_size, frame_bounds,
                             detectionColor(detection.signal_id), 1);
        }
        if (result.candidate.has_value()) {
            drawDetectionBox(canvas, *result.candidate, source_size, frame_bounds, kCandidateColor,
                             2, "Candidate");
        }
        if (result.active_detection.has_value()) {
            drawDetectionBox(canvas, *result.active_detection, source_size, frame_bounds,
                             kActiveColor, 3, "Active");
        }
    } else {
        cv::putText(canvas, "Aguardando frame da fonte", {frame_content.x + 18, frame_content.y + 42},
    ...
    if (!result.debug_roi_frame.empty()) {
        ...
        for (const auto &detection : result.raw_detections) {
            drawDetectionBoxOnRoiTile(canvas, detection, source_size, roi_bounds,
                                      detectionColor(detection.signal_id), 1);
        }
        if (result.candidate.has_value()) {
        ...
        drawLabelTag(canvas, {roi_content.x + 10, roi_content.y + 22},
                     std::to_string(source_size.width) + "x" + std::to_string(source_size.height),
                     kRoiColor);
    } else {
    ...
    cv::putText(canvas, "Diagnostico do backend ativo", {footer_area.x + 16, footer_area.y + 28},
                cv::FONT_HERSHEY_SIMPLEX, 0.58, kTextPrimary, 1, cv::LINE_AA);
    int footer_y = footer_area.y + 54;
    std::vector<std::string> footer_blocks = {
        "Labels do modelo: " +
            (result.model_labels_summary.empty() ? std::string("n/a") : result.model_labels_summary),
        "Input do modelo: " +
            (result.model_input_summary.empty() ? std::string("n/a") : result.model_input_summary),
        "Sinais canonicos emitidos em WebSocket: stop, turn_left, turn_right.",
        "Labels desconhecidas permanecem em raw_detections e nao geram signal:detected.",
    };
    for (const auto &block : footer_blocks) {
        for (const auto &line : wrapText(block, 126)) {
